...
public:
    ...
    bool Insert(const DATA_TYPE *item, const int KEY_LEN){
        for(uint32_t i = 0;i < hash_num;++i){
            uint32_t pos = this->hash(item->ItemID, KEY_LEN, i) % length;
            counters[i][pos] += !bitsets[i]->SetNGet(pos);;
        }
        return 0;
    }

    COUNT_TYPE Query(const DATA_TYPE *item, const int KEY_LEN){
